...
5. **Склад и закупка**  
   ...
   - в разделе “Склад/Закупка” видны дефицитные позиции и заявки.

6. **Аналитика и отчеты**  
   Директор открывает “Аналитика”:
   - видит графики выручки;
   - топ-клиентов и топ-товары;
   - показатели по менеджерам/подразделениям (в дальнейшем);
   - может выгрузить отчёт или переключаться по периодам.

7. **Управление правами**  
   Директор заходит в “Разрешения”:
   - выбирает сотрудника;
   - снимает галочки с опасных действий (удаление заказов, правка оплаченных, экспорт фин. отчетов);
   - изменения тут же применяются — backend проверяет права при каждом запросе.
