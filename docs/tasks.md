...
- #### 8.1. Дизайн-система
  ...
  - [x] Финализировать палитру и токены для светлой/тёмной темы.
  - [x] Описать sizes для шрифтов (H1, H2, body, caption).
  - [x] Выстроить систему отступов и радиусов.

- #### 8.2. Унификация компонентов

  - [x] Вынести общие компоненты:
    - [x] `AppButton`, `AppInput`, `AppSelect`, `AppTable`, `AppCard`, `AppModal`, `AppDrawer`, `AppToast`.
  - [x] Пройтись по всем экранам и заменить “разрозненный” UI на общие компоненты.

- #### 8.3. Состояния и валидация

  - [x] Добавить лоадеры/скелетоны на основных экранах (дэшборд, заказы, касса).
  - [x] Унифицировать отображение ошибок (тосты + подсветка полей).
  - [x] Маска телефона `+7 (XXX) XXX-XX-XX` + валидация длины.
  - [x] Подтверждения для опасных действий (удаление, отмена, возврат).

- #### 8.4. UX и доработки по ролям

  - [x] Отдельные “виды” дэшборда по ролям (или хотя бы разные наборы виджетов).
  - [x] Проверка ключевых сценариев:
    - [x] дизайнер: создание/отправка заказа;
    - [x] производство: просмотр очереди и смена статусов;
    - [x] директор: просмотр аналитики/финансов.
  - [x] Микро-анимации (открытие drawer’ов, hover’ы, drag’n’drop в канбане).

- #### 8.5. Testing

  - [x] Базовые backend-тесты (авторизация, создание заказа, оплата).
  - [x] Smoke-тесты фронта (ручные чек-листы).
  - [x] Прогон полного цикла:
    - [x] дизайнер создаёт заказ → отправляет в производство;
    - [x] производство выполняет → директор/касса принимает оплату;
    - [x] заказ попадает в аналитику.
