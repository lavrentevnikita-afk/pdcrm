...
- #### 6.2. Frontend: экран “Касса”
  ...
  - [x] `CashView`:
    - [x] панель фильтров (телефон, дата исполнения, статус оплаты);
    - [x] таблица заказов (ID, клиент, сумма, статус, остаток к оплате).
  - [x] Модалка “Оплата заказа”:
    - [x] поля: сумма к оплате (по умолчанию остаток), способ оплаты;
    - [x] кнопка “Провести оплату”;
    - [x] отображение ошибок/успеха (тосты).
  - [x] Блок “Итоги”:
    - [x] сумма оплат за текущую смену/день.
