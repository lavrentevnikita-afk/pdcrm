...
- #### 1.1. Репозиторий и структура
  ...
  - [ ] Добавить `.gitignore` для Node/Vue/SQLite.
  - [ ] Создать корневую структуру:
    - [ ] `backend/`
    - [ ] `frontend/`
    - [ ] `docs/` (архитектура, таски, запуск).

- #### 1.2. Backend: базовый скелет

  - [ ] Инициализировать `backend` (`npm init`).
  - [ ] Установить пакеты:
    - [ ] `express`, `cors`, `morgan`, `dotenv`;
    - [ ] `sqlite3`, `knex`;
    - [ ] `jsonwebtoken`, `bcrypt` (или `bcryptjs`);
    - [ ] `express-validator`;
    - [ ] `nodemon` (dev).
  - [ ] Создать `src/server.js` с:
    - [ ] загрузкой `.env`;
    - [ ] подключением middleware (`cors`, `json`, `morgan`);
    - [ ] заглушкой маршрута `/api/health`.
  - [ ] Настроить `knexfile.js` и подключение к SQLite.
  - [ ] Добавить npm-скрипты: `dev`, `start`, `migrate`, `seed`.

- #### 1.3. Frontend: базовый скелет

  - [ ] Создать `frontend` через `npm create vite@latest` (Vue).
  - [ ] Установить:
    - [ ] `vue-router`,
    - [ ] `pinia`,
    - [ ] `axios`.
  - [ ] Подключить глобальные стили (`main.css` или Tailwind).
  - [ ] Настроить базовый `router`:
    - [ ] маршруты `/login`, `/dashboard`, `/orders` и т.д. (заглушки).
  - [ ] Добавить пустые страницы-компоненты:
...
- #### 1.4. Цветовая палитра и тема
  ...
  - [ ] Завести CSS-переменные для палитры:
    - [ ] серые (4 уровня),
    - [ ] светлый голубой,
    - [ ] яркий голубой,
    - [ ] насыщенный синий,
  ...
  - [ ] Настроить 2 набора токенов: `light` и `dark` тема (через классы `theme-light`/`theme-dark` на `<body>`).

- #### 1.5. Глобальный лейаут (UI-каркас)

  - [ ] Создать компонент `MainLayout`:
    - [ ] левый сайдбар;
    - [ ] верхний хэдер;
    - [ ] область контента.
  - [ ] Внедрить `MainLayout` в `App.vue` (кроме `/login`).
  - [ ] Сверстать **левое меню**:
    - [ ] логотип/название (“Печатный двор — внутренняя CRM система”);
    - [ ] группы меню: Основные / Управление / Другое;
    - [ ] пункты: Дэшборд, Заказы, Касса, Склад, Персонал, Аналитика, Справочники, Настройки, Разрешения;
    - [ ] поиск по меню (пока без логики или простая фильтрация).
  - [ ] Добавить внизу меню:
    - [ ] версию приложения (`PDCRM v0.1 phase1`);
    - [ ] кнопку “Выход” (заглушка).
...
- #### 3.1. Backend: данные для дэшборда
  ...
  - [ ] Миграция `orders` (минимальный набор полей: сумма, статус, дедлайн, флаг горящего).
  - [ ] Endpoint `GET /api/dashboard/summary`:
    - [ ] общая сумма заказов за период;
    - [ ] количество активных/завершенных;
    - [ ] список “горящих” (ближайшие дедлайны);
    - [ ] данные для графика загрузки (по дням недели).
  - [ ] Написать простые seed-данные (несколько заказов разных статусов).
...
- #### 3.2. Frontend: карточки и график
  ...
  - [ ] Компонент `DashboardTotalCard`:
    - [ ] большая синяя карточка “Сумма заказов”;
    - [ ] отображение суммы и процента изменения.
  - [ ] Компоненты `DashboardStatCard`:
    - [ ] “Активные заказы”;
    - [ ] “Завершенные заказы”;
    - [ ] селект/кнопка выбора периода (например, текущий месяц).
  - [ ] Компонент `DashboardWorkloadChart`:
    - [ ] линейный график “Загруженность” (Mon–Sun);
    - [ ] пока можно взять статичные данные с последующей заменой на API.
  - [ ] Компонент `DashboardHotOrders`:
    - [ ] список карточек “горящих заказов”;
    - [ ] название, номер, дата дедлайна красным;
...
- #### 5.3. Frontend: калькулятор в drawer
  ...
  - [ ] Компонент `OrderItemRow`:
    - [ ] селект товара (поиск по названию + выбор категории);
    - [ ] поле тиража/количества;
    - [ ] отображение цены за единицу и итога по строке;
    - [ ] поля ручной скидки (% или сумма);
    - [ ] кнопка удаления строки.
  - [ ] В `OrderCreateDrawer`:
    - [ ] кнопка “Продукция +” → добавляет строку `OrderItemRow`;
    - [ ] отдельный блок для постпечатки (можно отдельный справочник/категорию);
    - [ ] подсчёт:
      - [ ] сумма по всем позициям;
      - [ ] поле скидки на весь заказ;
      - [ ] итог к оплате.
  - [ ] Сохранение:
    - [ ] отправка `order` + `order_items` на backend;
